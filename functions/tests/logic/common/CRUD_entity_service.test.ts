...
describe("Emission factors - CRUD operations", () => {
  ...
  ])("Create an entity of type %s", async (type, entityName, collectionName, validationSchema, testData) => {
    jest.spyOn(FirestoreUtil, "createWithCustomId").mockResolvedValue(testData[0]);

    expect(await CRUDEntityService.createEntity(testData[0], type as EntityType)).toStrictEqual(testData[0]);
  });
